...
const LoginModal = ({ isOpen, closeModal, active: propsActive }) => {
  const [active, setActive] = useState(propsActive);
  const { reset: setMobile, ...mobile } = useInput("");
  const { reset: setPassword, ...password } = useInput("");
  const { reset: setConfirmPassword, ...confirmPassword } = useInput("");
  ...
  const loginZ = z.object({
    phoneNumber: z.string().length(10, "Incorrect phone number"),
  ...
  const forgotPasswordHandler = async (e) => {
    ...
    if (!data.success) return toast.error(data.error.issues[0].message);
    await auth.changePassword(data.data);
    setActive(Action.LOGIN);
  };
    ...
    <Dialog onClose={closeModal} open={isOpen}>
      ...
      <LoaderBG showLogo={false}>
        <div className="fixed inset-0 overflow-y-auto z-10">
          <div className="flex items-center justify-center min-h-full">
            <Dialog.Panel className="bg-primary rounded-xl w-[90vw] max-w-[500px]">
              <header className="grid grid-cols-2 text-xl">
                <button className={`h-14 ${active === Action.SIGN_UP ? "text-white" : "bg-secondary text-black"}`} onClick={() => setActive(Action.SIGN_UP)}>Signup</button>
                <button className={`h-14 ${active !== Action.SIGN_UP ? "text-white" : "bg-secondary text-black"}`} onClick={() => setActive(Action.LOGIN)}>Login</button>
                {active === Action.FORGOT_PASSWORD && (
                  <button className="absolute right-10 top-1/2 -translate-y-1/2" onClick={() => setActive(Action.LOGIN)}><FiX className="w-5 h-5" /></button>
                )}
              </header>

              <div className="scrollbar overflow-auto pb-10 px-4">
                <button className="text-white absolute right-10 top-7 md:hidden" onClick={closeModal}><FiX className="h-6 w-6" /></button>
                <div className="pt-8 pb-6"><Logo /></div>

                {active === Action.LOGIN && (
                  <form onSubmit={loginHandler} className="w-full">
                    <Input prefix="+91" type="number" id="mobile" placeholder="10 Digit Phone number" maxLength={10} {...mobile} />
                    <Input type="password" id="password" placeholder="Password" className="mt-5" {...password} />
                    <div className="text-right">
                      <button type="button" className="underline mt-4 text-white" onClick={() => setActive(Action.FORGOT_PASSWORD)}>Forgot password?</button>
                    </div>
                    <div className="mt-8">
                      <button type="submit" className="bg-accent uppercase text-xl text-white w-full h-12 rounded-md shadow-md hover:shadow-lg">Login</button>
                    </div>
                  </form>
                )}
                ...
                {active === Action.SIGN_UP && (
                  <form onSubmit={signUpHandler}>
                    <Input prefix="+91" type="number" id="mobile" placeholder="10 Digit phone number" maxLength={10} {...mobile} />
                    <Input type="password" id="password" placeholder="Password" className="mt-5" {...password} />
                    <Input id="confirm-password" type="password" placeholder="Confirm Password" className="mt-5" {...confirmPassword} />
                    <button type="submit" className="bg-accent uppercase text-xl text-white w-full h-12 rounded-md shadow-md hover:shadow-lg mt-8">SIGNUP</button>
                  </form>
                )}
                ...
                {active === Action.FORGOT_PASSWORD && (
                  <form onSubmit={forgotPasswordHandler} className="w-full">
                    ...
                    <Input id="confirm-password" type="password" placeholder="Confirm Password" className="mt-5" {...confirmPassword} />
                    <div className="mt-8">
                      <button type="submit" className="bg-accent uppercase text-xl text-white w-full h-12 rounded-md shadow-md hover:shadow-lg">Change password</button>
                    </div>
                  </form>
                )}
              </div>
