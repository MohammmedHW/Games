...
export default function Team() {
  ...
  //       }
  //       if (user.access && !Object.values(user.access).some((val) => val)) {
  //         throw new Error("At least one access must be selected for the admin");
  //       }
  ...
  //       await makeRequest("/team/", "POST", body);
  ...
  //       await makeRequest(`/team/${user.id}/`, "PUT", body);

  //       // Update local state
  //       setTeam(team.map((u) => (u.id === user.id ? user : u)));
  //       setShowTeamModal(false);
  //       toast.success("Team member updated successfully!");
  //     } catch (error) {
